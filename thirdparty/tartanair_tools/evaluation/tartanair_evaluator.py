...
def plot_traj(gtposes, estposes, vis=False, savefigname="/home/honsen/qwe1.png", title=''):
    ...
    gt = np.array(gtposes)
    est = np.array(estposes)
    # cm = plt.cm.get_cmap('Spectral')

    ax.plot(gt[:,2],gt[:,1],gt[:,0], linestyle='dashed',c='k')
    ax.plot(est[:, 2], est[:, 1], est[:, 0], c='#ff7f0e')
