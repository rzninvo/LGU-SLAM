...
class DeformableAttention2D(nn.Module):
    ...
    def forward(self, x_h, x_cur, return_vgrid = False):
        """
        b - batch
        h - heads
        x - height
        ...
        heads, b, h, w, downsample_factor, device = self.heads, x_h.shape[0], *x_h.shape[-2:], self.downsample_factor, x_h.device
...
if __name__ =="__main__":
    ...
    convz = nn.Conv2d(256, 128, 3, padding=1)
    total_params = sum(p.numel() for p in convz.parameters() if p.requires_grad)
