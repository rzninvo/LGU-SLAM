...
class FactorGraph:
    ...
    def update(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, motion_only=False):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            motn = torch.cat([coords1 - self.coords0, self.target - coords1], dim=-1)#use the correspondence field to derive optical flow induced by camera motion as the difference pij-pj.Furthermore,the residual from the previous BA Solution is concated with the flow field.
...
if __name__ == '__main__':
    ...
    jj = [1,1,0]
    if not isinstance(ii, torch.Tensor):
        ii = torch.as_tensor(ii, dtype=torch.long, device='cpu')

    if not isinstance(jj, torch.Tensor):
        jj = torch.as_tensor(jj, dtype=torch.long, device='cpu')
