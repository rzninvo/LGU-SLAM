...
if __name__ == '__main__':
    ...
    """
    Upload to official website for further testing
    """

    image_path = os.path.join(args.datapath, 'rgb')
    images_list = sorted(glob.glob(os.path.join(image_path, '*.png')))[::stride]
    tstamps = [float(x.split('/')[-1][:-4]) for x in images_list]
    traj_est = traj_est.tolist()
    with open("xxx.txt", "w") as file:
        for i in range(len(traj_est)):
            traj_est[i].insert(0, tstamps[i])
            traj_est[i] = [str(x) for x in traj_est[i]]
            file.write(" ".join(traj_est[i]) + "\n")
